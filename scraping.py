...
#     user["place"]=data[X]
#     user["date"]=data[X]
#     user["courtNum"]=data[X]
#     user["yyyymmdd"]=data[X]
#     user["priority"]=data[X]
#     users.append(user)
users = [
    {
        "name": "堀澤貴行",
...
for key, value in usersJson.items():
    ...
    for userInfo in userInfos:
        ...
        driver.find_element(
            By.XPATH,
            f'//*[@id="FRM_RSGK404"]/div[7]/div/div/a[{str(int(userInfo["courtNum"])+1)}]',
        ).click()
        time.sleep(1)
        # 日付選択
        ...
        driver.find_element(
            By.ID,
            f'idinput_{places[userInfo["place"]].rstrip("0")}{str(int(userInfo["courtNum"])-1).zfill(2)}_{userInfo["yyyymmdd"]}_{userInfo["time"]}',
        ).click()
        time.sleep(1)
        # 「申し込む」選択
        driver.find_element(
